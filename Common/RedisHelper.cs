...
namespace Common
{
    ...
    public class RedisHelper
    {
        ...
        static RedisHelper()
        {
            ConnectionString = ConfigurationManager.ConnectionStrings["RedisConn"].ConnectionString;
            _connMultiplexer = ConnectionMultiplexer.Connect(ConnectionString);
        ...
        {
            redisKey = AddKeyPrefix(redisKey);
            var json = JsonHelper.SerializeObject(redisValue);
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public T StringGet<T>(string redisKey, TimeSpan? expiry = null) where T:class
        {
            redisKey = AddKeyPrefix(redisKey); //加key标识
            string jsonStr = _db.StringGet(redisKey);//取出json字符串
            if(string.IsNullOrEmpty(jsonStr))  
            { //redis中不存在
                return null;
            }
            //反序列化
            return JsonHelper.DeserializeObject<T>(jsonStr);
        }
